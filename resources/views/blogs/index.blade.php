                                ...
                                <tr class="border-t">
                                    ...
                                    <td class="py-2 px-4">
                                        <a href="{{ route('blogs.show', $blog->id) }}"
                                            class="bg-blue-500 text-white py-1  my-1 px-2 rounded hover:bg-blue-400">{{ __('View') }}</a>
                                        <a href="{{ route('blogs.edit', $blog->id) }}"
                                            class="bg-yellow-500 text-white  my-1 py-1 px-2 rounded hover:bg-yellow-400">{{ __('Edit') }}</a>
                                        <form action="{{ route('blogs.destroy', $blog->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="bg-red-500 text-white py-1 px-2 rounded hover:bg-red-400 my-1"
